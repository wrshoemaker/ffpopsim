# Benefits of Sex #
The benefits of sex are most pronounced when there are many loci at which mutations segregate. Hence the high\_d part of FFPopSim is most appropriate. The following script accepts command line arguments that specify the population parameters (all of which have default values),
and then evolves the population for a specified time. Along the way, the mean fitness, the fitness variance and the number of distinct genotypes are recorded and plotted in the end.

```

# Import module
import sys
sys.path.append('../pkg/python')  #set this to where ever the library is
